# bash completion for incus-box
# Source this file or install to /etc/bash_completion.d/incus-box
# or ~/.local/share/bash-completion/completions/incus-box
    ...
    case "${cmd}" in
        create)
            ...
            case "${prev}" in
                --image|-i)
                    # shellcheck disable=SC2207
                    COMPREPLY=( $(compgen -W "$(_incus-box_images)" -- "${cur}") )
                    return ;;
                --storage-pool)
                    # shellcheck disable=SC2207
                    COMPREPLY=( $(compgen -W "$(_incus-box_storage_pools)" -- "${cur}") )
                    return ;;
                --btrfs-compress)
                    # shellcheck disable=SC2207
            ...
            case "${prev}" in
                enter|--user|-u)
                    if [ "${prev}" = "enter" ] || [ "${prev}" = "${cmd}" ]; then
                        # shellcheck disable=SC2207
                        COMPREPLY=( $(compgen -W "$(_incus-box_containers)" -- "${cur}") )
                    fi
                    return ;;
            esac
            # shellcheck disable=SC2207
...
complete -F _incus-box incus-box
