...
_incus-box() {
    ...
    case "${state}" in
        command)
            local commands=(
                'create:Create a new incus-box container'
                'enter:Enter an existing incus-box container'
                'list:List incus-box containers'
                'ls:List incus-box containers (alias)'
                'rm:Remove an incus-box container'
                'remove:Remove an incus-box container (alias)'
                'stop:Stop one or more incus-box containers'
                ...
                'setup-rootless:Configure rootless incus-box via incus-user'
                'version:Show version'
                'help:Show help'
            )
            _describe 'incus-box commands' commands
            ;;
            ...
            case "${line[1]}" in
                create)
                    _arguments \
                        ...
                        {-h,--help}'[Show help]'
                    case "${state}" in
                        images) _describe 'images' "$(_incus-box_images)" ;;
                        pools)  _values 'pools' $(_incus-box_storage_pools) ;;
                    esac
                    ;;
                ...
                enter)
                    ...
                    case "${state}" in
                        containers) _values 'containers' $(_incus-box_containers) ;;
                    esac
                    ;;
                ...
                upgrade)
                    _arguments \
                        '*:container:->containers' \
                        {-a,--all}'[Upgrade all incus-box containers]' \
                        {-r,--root}'[Use rootful incus]' \
                        {-v,--verbose}'[Show verbose output]' \
                        {-V,--version}'[Show version]' \
                        {-h,--help}'[Show help]'
